...
Q = torch.diag(torch.FloatTensor([1.0, 1.0, 0.5, 0.5]))  # state cost matrix
...
def sgd_mpc_control(xref, x0, _, oa, od):
    ...
    x0_t = torch.tensor(x0, device=device, requires_grad=False, dtype=torch.float32)
    oa_t = torch.tensor(oa, device=device, requires_grad=True, dtype=torch.float32) 
...
def do_simulation(cx, cy, cyaw, ck, sp, dl, initial_state):
    ...
    while MAX_TIME >= time:
        ...
        oa, odelta, ox, oy, oyaw, ov = sgd_mpc_control(
